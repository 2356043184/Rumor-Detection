...
class CLIP4Clip(CLIP4ClipPreTrainedModel):
    def __init__(self, cross_config, clip_state_dict, task_config):
        ...
        if self.sim_header == "seqLSTM" or self.sim_header == "seqTransf":
            self.frame_position_embeddings = nn.Embedding(cross_config.max_position_embeddings, task_config.video_dim)
        if self.sim_header == "seqTransf":
            self.transformerClip = TransformerClip(width=task_config.video_dim, layers=self.task_config.cross_num_hidden_layers,
                                                   heads=transformer_heads, )
        if self.sim_header == "seqLSTM":
            self.lstm_visual = nn.LSTM(input_size=task_config.video_dim, hidden_size=task_config.video_dim,
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, video, video_mask=None):
        ...
        b, pair, ts, video_dim = video.shape
        video = video.view(b * pair * ts, video_dim)
        video_frame = ts
    ...
    def get_visual_output(self, video, video_mask, shaped=False, video_frame=-1):
        ...
        visual_hidden = video.view(bs_pair, -1, video.size(-1))
    ...
    def _loose_similarity(self, sequence_output, visual_output, attention_mask, video_mask, sim_header="meanP"):
        ...
        visual_output = visual_output / (visual_output.norm(dim=-1, keepdim=True) + 1e-9)
    ...
    def get_visual_sim_output(self, visual_output, video_mask, sim_header="meanP", shaped=False):
        if shaped is False:
            video_mask = video_mask.view(-1, video_mask.shape[-1])
        visual_output = visual_output.contiguous()
        assert sim_header in ['seqLSTM', 'seqTransf']
