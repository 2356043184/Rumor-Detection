...
class CLIP4Clip(CLIP4ClipPreTrainedModel):
    ...
    def get_visual_grid_output(self, video, video_mask, shaped=False, video_frame=-1):
        if shaped is False:
            video_mask = video_mask.view(-1, video_mask.shape[-1])
            video = torch.as_tensor(video).float()
            b, pair, bs, ts, channel, h, w = video.shape
        ...
        bs_pair = video_mask.size(0)
        visual_hidden = self.clip.encode_image_grid(video, video_frame=video_frame).float()
        visual_hidden = visual_hidden.view(bs_pair, -1, 50, visual_hidden.size(-1))
    ...
    def get_visual_sim_grid_output(self, visual_output, video_mask, sim_header="meanP", shaped=False):
        if shaped is False:
            video_mask = video_mask.view(-1, video_mask.shape[-1])
        visual_output = visual_output.contiguous()
        if sim_header == "meanP":
